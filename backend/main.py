...
async def setup_documentation(request: SetupRequest):
    ...
    if request.should_setup_new:
        ...
        return StreamingResponse(content=message_stream("session exists"), media_type="text/event-stream")
    else:
        return StreamingResponse(content=message_stream("no session exists"), media_type="text/event-stream")
...
async def query_documentation(request: QueryRequest):
    ...
    results = retrieve_from_store(question=question, session_id=session_id, n_points=n_points)
    payload = [{"page": {int(r.id)+1}, "content": {r.payload['document']}, "source_link": {r.payload['source_link']}} for r in results]
